...
def make_meshgrid(lims_x = (-2.5, 2.5), lims_y = (-8, 2), num_points = 100 ):
    ...
    xx, yy = np.meshgrid(np.linspace(*lims_x, num_points), np.linspace(*lims_y, num_points))
    grid = np.stack((xx, yy), axis=-1).reshape(-1, 2)

    return torch.tensor(grid), xx, yy

def get_log_probs_banana(mean, cov, grid, m, n):
    """Evaluate banana density over a meshgrid with dimension m,n"""
    encoder_true = UIEncoder(mean = mean, cov =  cov, num_eps_samples =  grid.shape[0], dim_eps = 3, dim_z = 2)
...
def plot_samples(z_samples, xx, yy, log_probs):
    ...
    axs[0].scatter(z_samples[:, 0], z_samples[:, 1], color='red', s = 3, alpha = 1, label = "UIVI samples")
