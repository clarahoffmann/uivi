...
class UIEncoder(L.LightningModule):
    def __init__(self, mean, cov, num_eps_samples=1, dim_eps = 3, dim_z = 2, T = 5, L = 5, latent_dim = 100):
        super().__init__()
        self.encoder_mu_sigma = nn.Sequential(nn.Linear(dim_eps, latent_dim), nn.ReLU(), 
        ...
        self.dist = MultivariateNormal(loc = self.mean_batched,
                                    covariance_matrix = self.cov_batched)
        
        self.hmc = HMC(dim_z, T, L)
        
    def forward(self, epsilon = None):
        # 1. Sample input variable
        if epsilon is None:
            epsilon = torch.randn(self.num_eps_samples, self.dim_eps).requires_grad_(False)
    ...
    def HMC_bound(self, epsilon, z_samples, mu_sample, sigma_sample):
        Dz = MultivariateNormal(loc = mu_sample, covariance_matrix = torch.diag_embed(sigma_sample**2))
